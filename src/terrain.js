...
export const terrain = (function() {
  ...
  const _MIN_CELL_RESOLUTION = 96;//độ phân giải cua planet, (sau này sử dụng để làm sao khi càng tới gần planet thì độ phân giải càng cao)
  ...
  class TerrainChunkManager {
	...
	/*
	cập nhật các mảnh đất hiển thị bằng cách sử dụng cấu trúc quadtree để quản lý 
	và tối ưu hóa các mảnh đất dựa trên vị trí và khả năng hiển thị của camera. 
	Nó tính toán sự khác biệt giữa các mảnh đất cũ và mới, loại bỏ các mảnh không 
	cần thiết và tạo các mảnh mới cho phần hiển thị của địa hình.
	*/
    _UpdateVisibleChunks_Quadtree() {
		//nhận một đối tượng c và trả về một chuỗi duy nhất dựa trên vị trí và kích thước của đối tượng, 
		//được sử dụng làm khóa trong các từ điển (dictionaries) sau này
      function _Key(c) {
	...
	//Khởi tạo quadtree
      ...
      const sides = q.GetChildren();//các mặt

	//tạo và cập nhật các mảnh địa hình mới
      let newTerrainChunks = {};//lưu trữ thông tin về các mảnh địa hình mới được tạo ra.
      const center = new THREE.Vector3();//tọa độ trung tâm của một mảnh địa hình
      const dimensions = new THREE.Vector3();//lưu trữ thông tin về kích thước của một mảnh địa hình.
      for (let i = 0; i < sides.length; i++) {
        this._groups[i].matrix = sides[i].transform;
        this._groups[i].matrixAutoUpdate = false;
        for (let c of sides[i].children) {
          c.bounds.getCenter(center);//alert(center.x+"-"+center.y+"-"+center.z);
	  ...
	  //các mảnh đất cũ cần được loại bỏ
	...
	//Với mỗi mảnh đất mới, tạo một offset (vị trí đặt) 
	//dựa trên vị trí của mảnh đất và kích thước của nó
...
//Gán giá trị của newTerrainChunks cho this._chunks để cập nhật danh sách mảnh đất hiện tại
