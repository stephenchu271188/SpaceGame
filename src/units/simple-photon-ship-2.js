...
const _radius=800;
let _ship_id=0;
class SimplePhotonShip2 extends SpaceShip {
	...
	constructor(params){
		...
		//this._game._noticeBoard.add_message("ShipID="+this._t_ship_id);
	}
	...
	launch_photon(_shipclass,_level,_color){//nhieu tia sang photon huong ve phia truoc
		...
		let _create_laser=()=>{
			...
			//_shipclass._game._graphics.clean_all_laser();
			this._laser_counter++;
			...
			//let _pos2=this._game._utils.findPointOnLine(this.Position,this._target_object.Position,2000);
			if(this._target_object&&!this._target_object.Dead)
				_pos2=this._game._utils.findPointOnLine(this.Position,this._target_object.Position,4000);
			else
				_shipclass.getFrontPos(5000);
			...
			let _update=(t)=>{
				...
				for(let i=0;i<_targets.length;i++){
					...
					if(typeof _target._hited_by_simple_photon_ship_2==='undefined'){
						_target._hited_by_simple_photon_ship_2=new Array();
					}
					else{
						let _found=false;
						for(let j=0;j<_target._hited_by_simple_photon_ship_2.length;j++){
							if(_target._hited_by_simple_photon_ship_2[j].ship_id===this._t_ship_id&&
							_target._hited_by_simple_photon_ship_2[j].laser_id===_laser_id){
								//alert("FOUND");
								_found=true;
								break;
							}
					...
					if(_d1+_d2<_ray_length+this._laser_range){
						...
						_target.Take_Damage(_shipclass,"Photon",null,this._damage);
						_target._hited_by_simple_photon_ship_2.push({ship_id:this._t_ship_id,laser_id:_laser_id});
						//this._game._noticeBoard.add_message("dam1="+_damage);
						//this._game._noticeBoard.add_message("dam2="+this._damage);
					}
				}};
			};
	...
	Fire(){//overwrite
		...
		if(!this._photon_color)this._photon_color="white";
