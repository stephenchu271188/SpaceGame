...
class SimpleDefenseShip extends SpaceShip {
		...
		let _update_shield=(timeElapsedS)=>{
			...
			if(!_targets){}
				...
				for(let i=0;i<_targets.length;i++){
					_target=_targets[i];
					if(!_target||_target===null||_target.Dead)
						continue;
					if(this.Position.distanceTo(_target.Position)<200){
						_target.SelfDestroy();
					}
				}
			}
