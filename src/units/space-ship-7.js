...
class SpaceShip7 extends SpaceShip {
	
	constructor(params){
		params.laser_color=new THREE.Color(247, 10, 2);
		params.shoot_delay=0.15;//thời gian delay giữa 2 lần bắn
		params.health=2300;
		params.damage=22;
		...
		//this._visibilityIndex = this._game._visibilityGrid.UpdateItem(
		  ...
		  this._rotate_speed=0.3;//van toc quay
		  ...
		  this._counter=0;
		  this._counter_mark=50;
		  this._axis_id=0;
		  this._axis_list=[this._utils.axisX,this._utils.axisY,this._utils.axisZ];
		  
		  this._t_min_distance=150;
		  this._t_max_distance=500;
	}
	...
	CheckTarget(timeInSeconds){
		...
		if(_distance<=this._t_min_distance){
		...
		if(this._lock_target===true){
			...
			this._counter++;
			if(this._counter>this._counter_mark){
				this._counter=0;
				this._axis_id++;
				if(this._axis_id>this._axis_list.length-1)
					this._axis_id=0;
			}
			const _axis=this._axis_list[this._axis_id];
			
			
			this._utils.rotateAboutPoint(this._virtual_object_2,_center,
										 _axis,timeInSeconds*this._rotate_speed*this._direct, false);
										
			const _obj_point=new THREE.Vector3();
			this._virtual_object_2.getWorldPosition(_obj_point);
			this._model.position.copy(_obj_point);
			
			const _target_pos_2=new THREE.Vector3(_player_pos.x-5,_player_pos.y+5,_player_pos.z);
		...
		if(_distance>this._t_min_distance&&_distance<this._t_max_distance){
