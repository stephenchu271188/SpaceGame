...
class SimpleIcyShip extends SpaceShip {
	...
	constructor(params){
		...
		this._recovery_time=6;//thời gian hồi phục
...
class SimpleIcyBall extends SpaceShip {
	...
	constructor(params){
		...
		this._radius_effect1=80;//khoảng cách mà khi tên lửa ở gần mục tiêu sẽ phát nổ
		this._radius_effect2=80;//bán kính tầm ảnh hưởng của vụ nổ
	...
	createSprite() {
		...
		const _image = this._game._image_preloader.getImage('particle12');
			...
			if(_targets.length>0){
				...
				if(this._last_time>=1){//1 giay 1 lan
					...
					for(let i=0;i<_targets.length;i++){
						...
						_target._params.blasterSystem._damage=(_target._damage*99)/100;//damage giam 1%
					}
