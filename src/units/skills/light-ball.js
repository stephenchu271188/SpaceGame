...
class LightBall extends Spacecraft{
	...
	constructor(params){
		...
		this._radius_effect1=50*this._game.rocket_multiplier;//khoảng cách mà khi tên lửa ở gần mục tiêu sẽ phát nổ
		this._radius_effect2=100*this._game.rocket_multiplier;//bán kính tầm ảnh hưởng của vụ nổ
		this._origin_position=this.get_world_position();//vị trí ban đầu
		this._max_distance=1900*this._game.rocket_multiplier;//quãng đường xa nhất tên lửa có thể bay đi
		_speed1=this._game._parameters._standard_rocket_speed*2.5;
	...
	SelfDestroy(){
		super.SelfDestroy();
		//this._params.game._entities['_explosionSystem'].Splode(this.Position,"turquoise","green",24,96);
	}
	
	cause_damage(timeInSeconds,_targets){
		for(let i=0;i<_targets.length;i++){
			...
			if(_distance>this._radius_effect1*3/4)_damage=this._params.damage/3;
			if(_distance>this._radius_effect1*1/2)_damage=this._params.damage/2;
					
			//_target.TakeDamage(timeInSeconds*_damage*2);
	...
	CheckTarget(timeInSeconds){
			...
			let _targets=this._game._unitMG.get_enemy_combat_unit_in_range_4(this._unit,_position,this._radius_effect1);
			this.cause_damage(timeInSeconds,_targets);
			
			if(_t_distance>this._max_distance){
				this.explode(_targets);
				this.SelfDestroy();
				return;
			}
			...
			if(_t_distance<50){
				...
				for(let i=0;i<_targets.length;i++){
					...
					if(_distance<30){
						this.explode(_targets);
						//this.SelfDestroy();
						this._params.damage=parseInt(this._params.damage/6);//giam sat thuong khi da cham muc tieu
					}
				}
...
class Sparks{
  constructor(params) {
	  ...
	  const _image = this._game._image_preloader.getImage('particle34');
