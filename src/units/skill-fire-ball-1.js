...
class FireBall_1{
	...
	init(){
		...
		this._game._graphics.Scene.add(root);
		root.position.copy(this._game._graphics.Camera.position);
		...
		cylinder();
		//animatBloom();
		this._game._graphics._composer2=bloomComposer;
		this._game.add_to_update_function_list((timeInSeconds)=>{
			try{
			//const _delta=timeInSeconds*500000;//console.log(_delta);
			_delta+=timeInSeconds*1000;console.log(_delta);
			
			material.uniforms.time.value = -_delta / (1000 * 2);
			material2.uniforms.time.value = -_delta / (3000 * 2);
			material3.uniforms.time.value = -_delta / (3000 * 2);
			...
			//this._game._graphics._threejs.setRenderTarget(null);
			//bloomComposer.render();
  
			//alert(...options.color5);
...
function postProc() {
	/*
	const renderScene=_game._graphics._renderPass;
	bloomPass=_game._graphics._bloomPass;
	bloomComposer=_game._graphics._composer;
	*/
	
  const renderScene = new RenderPass(_game._graphics.Scene, _game._graphics.Camera);
  ...
  //bloomComposer = new EffectComposer(_game._graphics.Renderer);
  //bloomComposer.addPass(renderScene);
  //bloomComposer.addPass(bloomPass);
  
  //_game._graphics._composer.addPass(renderScene);
  _game._graphics._composer.addPass(bloomPass);
}
...
function mesh() {
  ...
  //_game._graphics.Scene.add(mesh);
  root.add(mesh);
}
