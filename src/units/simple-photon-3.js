...
const _radius=800;
let _ship_id=0;
class SimplePhotonShip3 extends SpaceShip {//ban ra tung tia photon mot
	...
	constructor(params){
		...
		params.damage=params.game._parameters._standard_hp/150;
	...
	launch_photon(_shipclass,_level,_color){//phong lien tiep tia sang photon huong ve phia truoc
		...
		let _create_laser=()=>{
			...
			let _update=(t)=>{
				_shipclass._game._utils.translateObject(_laserObject,_pos2,t*this._laser_speed);
				...
				for(let i=0;i<_targets.length;i++){
					...
					if(typeof _target._hited_by_simple_photon_ship_3==='undefined'){
						_target._hited_by_simple_photon_ship_3=new Array();
					}
					else{
						let _found=false;
						for(let j=0;j<_target._hited_by_simple_photon_ship_3.length;j++){
							if(_target._hited_by_simple_photon_ship_3[j].ship_id===this._t_ship_id&&
							_target._hited_by_simple_photon_ship_3[j].laser_id===_laser_id){
								//alert("FOUND");
								_found=true;
								break;
							}
					...
					if(_d1+_d2<_ray_length+this._laser_range){
						...
						_target._hited_by_simple_photon_ship_3.push({ship_id:this._t_ship_id,laser_id:_laser_id});
					}
				}};
			};
