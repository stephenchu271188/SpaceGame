...
var _particle1='particle4';
var _particle2='particle4';
var _particle3='particle4';
//Tên lửa chùm, bắn 1 lần ra 5 quả
class Missile5 extends Rocket{
	constructor(params){
	...
	createSprite() {
		...
		//var map = new THREE.CanvasTexture(this._game._image_preloader.getImage(_particle1));
		const _image = this._game._image_preloader.getImage('particle4');
	...
	CheckTarget(timeInSeconds){
			...
			if(!this._ahead_point){
				this._ahead_point=this.get_back_point(750);
			}
			...
			if(_t_distance>100||_targets.length>0){
				...
				const _front_pos=this.get_ahead_point(50);
				...
				for(let i=0;i<_pos_list.length;i++){
					let _child_rocket= this._game._unitMG.create_uncombat_unit(ChildMissile5,this._model,this.Position,false);;
					/*
					if(this._lever_counter===1){
						_child_rocket= this._game._unitMG.create_uncombat_unit(Missile5,this._model,this.Position,false);
						_child_rocket._lever_counter++;
					}
					else{
						_child_rocket= this._game._unitMG.create_uncombat_unit(ChildMissile5,this._model,this.Position,false);
					}
						*/
...
class ChildMissile5 extends SpaceShip {//tàu thám hiểm
	...
	createSprite() {
        //var map = new THREE.TextureLoader().load("./resources/particle/noname-7.png");
		let _particle;
		if(this._lever_counter===1)_particle=_particle2;
		else _particle=_particle3;
		//var map = new THREE.CanvasTexture(this._game._image_preloader.getImage(_particle));
		const _image = this._game._image_preloader.getImage(_particle);
	  const texture = new THREE.Texture(_image);
	  texture.needsUpdate = true;
        var material = new THREE.SpriteMaterial({
            map: texture,
	...
	CheckTarget(timeInSeconds){
		const _position=this.get_world_position();
			...
			const _distance1=300;
			if(!this._ahead_point){
				this._ahead_point=this.get_back_point(550);
			}
			
			if(_t_distance<_distance1)
			{
				this.move_forward(timeInSeconds*_speed1);
			}	
			...
			//let _targets=this._game._unitMG.get_enemy_combat_unit_in_range(_position,this._radius_effect1);
			const _targets=this._game._unitMG.get_enemy_combat_unit_in_range_3(this,_position,this._radius_effect1);
			if(_targets.length>0||_t_distance>=_distance1){
				for(let i=0;i<_targets.length;i++){
					const _target=_targets[i][0];
					const _distance=_targets[i][1];
					...
					if(this._lever_counter===1){
						const _left_pos=this.get_left_point();
						const _right_pos=this.get_right_point();
						const _front_pos=this.get_ahead_point(500);
						const _bellow_pos=this.get_bellow_point();
						const _above_pos=this.get_above_point();
						const _pos_list=[
								findPointP4(_position,_left_pos,_front_pos),//phia truoc, ben trai
