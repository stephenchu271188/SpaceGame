...
class PlayerShip5 extends PlayerEntity{
	constructor(params){
		...
		params.max_health=params.game._parameters._standard_hp*3;
		...
		this._skills=['rain-of-bullets','light-shield','speed-up'];////speed-up phai de o cuoi cung
			...
			_engine_object.position.set(0,0,6.2);
	...
	init_skills(){//overwrite
		this._main_skill_ids=[5,1001,1002];
	...
	create_rain_of_bullets_icon(_shipclass){
		/*
		...
		//const _rocket_num=_shipclass._ship_package.get_speed_up_num();
		let _rocket_num_label=document.createElement("div");
		const _rocket_icon=document.createElement("img");
		_rocket_icon.src="./resources/icons/skills/Untitled6.png";
		...
		*/
		const _rocket_icon_container=_shipclass.create_skill_icon(_shipclass,"./resources/icons/skills/Untitled6.png");
		_shipclass._lock_continuous_rocket_skill=false;
		let _rocket_num=100;
		let _perform_fire_breathing_fc=()=>{
				...
				_shipclass.apply_rain_of_bullets_skill(_shipclass);
				
				_rocket_num--;
				
		};
		_shipclass.create_rain_of_bullets_icon._perform_fc=_perform_fire_breathing_fc;
		_rocket_icon_container.addEventListener("click",_perform_fire_breathing_fc);
		/*
...
class Sparks{
  constructor(params) {
	  ...
	  const _image = this._game._image_preloader.getImage('particle22');
	  const texture = new THREE.Texture(_image);
	  texture.needsUpdate = true;
    const uniforms = {
        diffuseTexture: {
            //value: new THREE.CanvasTexture(this._game._image_preloader.getImage('particle22'))
			value:texture
        },
        pointMultiplier: {
